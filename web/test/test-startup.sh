...
PREFIX="\033[0;36m[test-startup]\033[0m \033[0;36m"
SUFFIX="\033[0m"

DOCKER_SERVICE=test_web
...
# check if an existing container is running with the same name
if [ "$(docker ps -q -f name=$DOCKER_SERVICE)" ]; then
  echo -e "\033[1;31m[test-startup] container with name $DOCKER_SERVICE is already running\033[0m"
  exit 1
fi
# if .next dir is empty, we need to build the application first
if [ ! "$(ls -A ../.next)" ]; then
  echo -e "\033[1;31m[test-startup] .next dir is empty, please run 'yarn build' first\033[0m"
  exit 1
fi
